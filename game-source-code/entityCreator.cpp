...
void entityCreator::CreateEnemy()
{
	...
	if(_timeEnemy > (_generateEnemy - 1) && _timeEnemy < (_generateEnemy + 1) &&
	    Enemy::getTotalNumberofEnemies() < _MAXENEMIES) {
...
void entityCreator::CreateSatellite()
{
    ...
    if((fmod(_timeSatellite, 7) > 0) && fmod(_timeSatellite, 7) < (0.2) &&
        Satellite::getNumberofSatellitesAlive() == 0) {
...
{
    _timeSatellite = _entityClock->getSatelliteTime();
    if((fmod(_timeSatellite, 10) > 0) && fmod(_timeSatellite, 10) < (0.2) &&
        LaserGenerator::getNumberofLasersAlive() == 0) {
